...
///
/// Read-only. For writable tables (INSERT/UPDATE/DELETE), implement the
/// [`WritableTable`] trait and wrap it in a [`WritableTablePlugin`].
...
impl<T: WritableTable> WritableTablePlugin<T> {
    fn handle_insert(
        ...
        match self.inner.insert(request) {
    ...
    fn handle_update(
        ...
        match self.inner.update(request) {
            Ok(result) => mutation_response(result),
            Err(e) => {
    ...
    fn handle_delete(
        ...
        match self.inner.delete(request) {
            Ok(result) => mutation_response(result),
            Err(e) => {
        ...
        match req.get("action").map(String::as_str) {
            Some("generate") => match req.get("context") {
                Some(s) => match serde_json::from_str::<QueryContext>(s) {
                    Ok(ctx) => match self.inner.generate(ctx) {
